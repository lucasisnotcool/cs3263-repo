...
def run_ollama_label(
    *,
    ...
    # Keep this timeout tight; when API fails, a long CLI timeout can stall notebook runs.
    cli_timeout_seconds = max(30, min(int(timeout_seconds), 60))
    try:
        ...
        raise RuntimeError(
            ...
            "Ensure Ollama is running (`ollama serve`) and model is available (`ollama list`)."
        ) from exc
