...
ARTIFACTS_SUBDIR = Path("artifacts") / "diffusion_plus_detection"
...
class DiffusionReviewConfig:
    ...
    # Protect against exact duplicate leakage across split.
    enforce_text_disjoint_split: bool = True
...
def _project_root() -> Path:
    ...
    for root in [cwd, *cwd.parents]:
        if (root / "experiment_trust_fake_reviews_plus_detection").exists():
            return root
    raise FileNotFoundError(
        "Could not locate project root containing experiment_trust_fake_reviews_plus_detection/."
    )
...
def _normalize_text_key(text: str, label_truth: int) -> str:
    normalized = _clean_text(text).lower()
    token = f"{normalized}|{int(label_truth)}"
...
def _load_fake_review_dataset(dataset_path: Path, target_rows: int, random_state: int) -> pd.DataFrame:
    ...
    out = pd.DataFrame()
    out["text"] = raw_df["text_"].astype(str).fillna("").map(_clean_text)
    ...
    out["text_key"] = [
        _normalize_text_key(t, int(y)) for t, y in zip(out["text"].tolist(), out["label_truth"].tolist())
    ]
    return out


def _split_train_test(df: pd.DataFrame, config: DiffusionReviewConfig) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    if not config.enforce_text_disjoint_split:
        train_df, test_df = train_test_split(
            df,
            test_size=config.test_size,
            random_state=config.random_state,
            stratify=df["label_truth"],
        )
        overlap = int(len(set(train_df["text_key"]) & set(test_df["text_key"])))
        return train_df.reset_index(drop=True), test_df.reset_index(drop=True), {
            "split_mode": "row_stratified",
            "text_key_overlap": overlap,
        }

    key_label_df = (
        df[["text_key", "label_truth"]]
        .drop_duplicates()
        .groupby("text_key", as_index=False)
        .agg(label_truth=("label_truth", "first"))
    )

    if key_label_df["label_truth"].nunique() < 2:
        train_df, test_df = train_test_split(
            df,
            test_size=config.test_size,
            random_state=config.random_state,
        ...
        overlap = int(len(set(train_df["text_key"]) & set(test_df["text_key"])))
        return train_df.reset_index(drop=True), test_df.reset_index(drop=True), {
            "split_mode": "row_stratified_fallback",
            "text_key_overlap": overlap,
        }

    train_keys, test_keys = train_test_split(
        key_label_df["text_key"],
        test_size=config.test_size,
        random_state=config.random_state,
        stratify=key_label_df["label_truth"],
    )
    train_key_set = set(train_keys.tolist())
    test_key_set = set(test_keys.tolist())

    train_df = df[df["text_key"].isin(train_key_set)].copy().reset_index(drop=True)
    test_df = df[df["text_key"].isin(test_key_set)].copy().reset_index(drop=True)

    if train_df.empty or test_df.empty or train_df["label_truth"].nunique() < 2:
        ...
        overlap = int(len(set(fallback_train["text_key"]) & set(fallback_test["text_key"])))
        return fallback_train.reset_index(drop=True), fallback_test.reset_index(drop=True), {
            "split_mode": "row_stratified_fallback",
            "text_key_overlap": overlap,
        }
    ...
    return train_df, test_df, {
        "split_mode": "grouped_text_key",
        "text_key_overlap": overlap,
        "train_unique_text_keys": int(train_df["text_key"].nunique()),
        "test_unique_text_keys": int(test_df["text_key"].nunique()),
    }
...
def run_diffusion_review_experiment(config: DiffusionReviewConfig | None = None) -> dict[str, Any]:
    ...
    train_df, test_df, split_info = _split_train_test(phase_a_df, cfg)
...
def score_review_texts(
    ...
    *,
    ...
    sample_count = (
        inference_samples if inference_samples is not None else int(bundle["config"]["inference_samples"])
    )
        ...
        rows.append(
            {
                ...
                "label": "real" if float(p_real[i]) >= 0.5 else "fake",
            }
        )
    return rows
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the standalone diffusion fake-vs-real review model (no LLM, no BN)."
    )
    ...
    parser.add_argument(
        "--disable-text-disjoint-split",
        action="store_true",
        help="Allow identical text to appear in both train/test split (not recommended).",
    )
    parser.add_argument(
        "--score-text",
        action="append",
...
def main(argv: list[str] | None = None) -> int:
    ...
    cfg = DiffusionReviewConfig(
        ...
        enforce_text_disjoint_split=not bool(args.disable_text_disjoint_split),
